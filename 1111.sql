...
,case when exists	(
	select test.orders.user_id from test.orders
	where test.orders.user_id = test.AGE.user_id
	and year (current_date) - year (STR_TO_DATE(test.AGE.BIRTHDATE, '%d.%m.%Y'))  > 20
	and year (current_date) - year (STR_TO_DATE(test.AGE.BIRTHDATE, '%d.%m.%Y')) < 25)
	then 'yes' else '' 
end '20-25 лет'
